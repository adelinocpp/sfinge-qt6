...
std::vector<float> TextureRenderer::render(const std::vector<float>& ridgeMap,
    ...
    if (m_params.enableAntiAliasing) {
        ...
        qDebug() << "[TextureRenderer] Anti-aliasing aplicado ao ridge map";
    }
...
std::vector<float> TextureRenderer::applyAntiAliasing(const std::vector<float>& ridgeMap) const {
    ...
    qDebug() << "[TextureRenderer] Aplicando anti-aliasing (sigma=" << m_params.antiAliasingSigma << ")";
    ...
    qDebug() << "[TextureRenderer] Anti-aliasing aplicado com sucesso";
    return smoothed;
}
...
std::vector<float> TextureRenderer::addPores(const std::vector<float>& texturedRidges,
    ...
    qDebug() << "[TextureRenderer] Adicionando" << numPores << "poros avançados em" << ridgePixelCount << "pixels de crista";
    ...
    qDebug() << "[TextureRenderer]" << poresAdded << "poros renderizados com variações avançadas";
    return result;
}
...
std::vector<float> TextureRenderer::applyDirectionalLighting(const std::vector<float>& image,
    ...
    qDebug() << "[TextureRenderer] Aplicando iluminação direcional";
    qDebug() << "[TextureRenderer] Azimute:" << m_params.lightAzimuth
             << "° Elevação:" << m_params.lightElevation << "°";
    ...
    qDebug() << "[TextureRenderer] Iluminação direcional aplicada";
    return result;
}
...
std::vector<float> TextureRenderer::applyArtifacts(const std::vector<float>& image,
    ...
    qDebug() << "[TextureRenderer] Aplicando artefatos realistas";
    std::vector<float> result = image;
    ...
    if (m_params.enableMoisture) {
        ...
        qDebug() << "[TextureRenderer]" << numMoisture << "manchas de umidade aplicadas";
    }
    ...
    if (m_params.enableDirt) {
        ...
        qDebug() << "[TextureRenderer]" << numDirt << "manchas de sujeira aplicadas";
    }
    ...
    if (m_params.enableScars) {
        ...
        qDebug() << "[TextureRenderer]" << m_params.numScars << "cicatrizes aplicadas";
    }
    ...
    if (m_params.enableDryness) {
        ...
        qDebug() << "[TextureRenderer]" << m_params.numDryCracks << "rachaduras de ressecamento aplicadas";
    }
    ...
    if (m_params.enableSmudges) {
        ...
        qDebug() << "[TextureRenderer]" << numSmudges << "borrões aplicados";
    }

    qDebug() << "[TextureRenderer] Artefatos aplicados com sucesso";
    return result;
}
